    ...
    <article className="project-card group overflow-hidden rounded-lg border border-[#1f3b2f]/10 bg-white/75">
      <a className="block h-full" href={`/${locale}/projects/${project.slug}`}>
        ...
        <div className="flex min-h-[245px] flex-col p-5">
          <div className="flex items-center justify-between gap-4 text-[10px] font-bold tracking-[.12em] text-[#526e5a]">
            <span>CASE {String(index + 1).padStart(2, '0')}</span>
            <span className="text-right">{sanitizeVisibleText(project.category)}</span>
          </div>
          ...
          <div className="mt-5 flex flex-wrap gap-2">{project.tags.map((tag) => <span className="rounded bg-[#eef1ea] px-2.5 py-1 text-[10px] font-semibold text-[#486050]" key={tag}>{sanitizeVisibleText(tag)}</span>)}</div>
