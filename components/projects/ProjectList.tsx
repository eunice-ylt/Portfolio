...
export function ProjectList({ projects, locale }: { projects: Project[]; locale: Locale }) {
  ...
  return (
    <>
      <div className="mb-10 flex flex-wrap gap-2" role="group" aria-label="Project categories">
      ...
      <div className="grid gap-5 md:grid-cols-2 xl:grid-cols-4">{visible.map((project, index) => <ProjectCard key={project.id} project={project} locale={locale} index={index} />)}</div>
    </>
  );
}
